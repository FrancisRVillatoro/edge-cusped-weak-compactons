...
check("semiwidth L(A=1)", semiwidth(mpf(1)),
      mpf("1.915346448610"), "(114)")
check("edge coefficient (2E)^{1/4} at A=1", (2 * energy(mpf(1))) ** (mpf(1) / 4),
      mpf("1.074569931823"), "(115)")

# Critical amplitude and energy of the family: V(W) = W - W^{3/2}/3 peaks at W=4.
check("critical energy E_crit", energy(mpf(2)), mpf(4) / 3, "(94)")

# Exact constant of the logarithmic width divergence, Eq. (92).
C_star = -6 * sqrt(2) + 2 * sqrt(6) + 2 * sqrt(2) * log(24 * (2 - sqrt(3)))
check("width constant C_*", C_star, mpf("1.677672331176593"), "(98)")

# Remainder of Eq. (97): [L(A) - 2 sqrt(2) log(1/eps) - C_*]/eps -> sqrt(2)/3.
eps = mpf("1e-5")
rem = (semiwidth(2 - eps) - (2 * sqrt(2) * log(1 / eps) + C_star)) / eps
check("O(eps) remainder of Eq. (97)", rem.real, sqrt(2) / 3, "(B11)", tol=mpf("1e-4"))

# Small-amplitude expansion, Eq. (92): L(A) = sqrt(2) A [1 + (pi/8 - 1/6) A + ...].
A = mpf("1e-4")
coef = (semiwidth(A) / (sqrt(2) * A) - 1) / A
check("small-amplitude coefficient", coef.real, pi / 8 - mpf(1) / 6, "(92)",
      tol=mpf("1e-4"))
...
check("semiwidth of the r^{2/3} compacton", L_balanced, 4 * pi, "(111)")

# The same width from the incomplete-beta representation, Eqs. (44)-(46).
...
check("same width via incomplete beta", L_beta, 4 * pi, "(44)")

# Edge law U ~ (3r/2)^{2/3}: check the amplitude A_0 of Eq. (37).
# s = a + b - p = 3, gamma = 2/s = 2/3, A_0 = [(1 - r_p/2) sqrt(K_p)]^{2/s}.
A0 = ((1 - r_p / 2) * sqrt(K_p)) ** (mpf(2) / 3)
check("edge amplitude A_0", A0, (mpf(3) / 2) ** (mpf(2) / 3), "(37)")
...
check("root W_* of the critical example", findroot(Fcrit, mpf("0.9")), 1, "(76)")
L_crit = quad(lambda W: 1 / sqrt(Fcrit(W)), [0, mpf("1e-8"), mpf("1e-3"), mpf("0.5"), 1])
check("semiwidth of the critical example", L_crit, mpf("1.695756643916"), "(78)")
